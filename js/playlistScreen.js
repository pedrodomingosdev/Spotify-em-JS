...
function chooseAlbum(name, xValue, cor) {
    ...
    const songDurations = [];
    totalAlbums = Math.floor(Math.random() * 20) + 1;

    for (let x = 1; x <= totalAlbums; x++) {
        const min = Math.floor(Math.random() * 3) + 1;
        const sec = Math.floor(Math.random() * 60);
        const formattedSec = sec.toString().padStart(2, '0');

        const durationInSeconds = (min * 60) + sec;
        songDurations.push(durationInSeconds);

        listSongs.innerHTML +=
            `<br />    
            <div class="song-item" >
                <div id="chooseSong"class="song-left" onclick="chooseSong(${x}, '${cor}')">
                    <span class="song-number">${x}</span>
                    <div class="song-info">
                        <p class="song-title">Song #${x}</p>
                        <small class="song-artist">Artist ${xValue}</small>
                    </div>
                    </div>
                    <div class="song-right">
                        <span class="song-duration">${min}:${formattedSec}</span>
                    </div>
                </div>
            </div>
            <p class="line-playlist"></p>`

    }

    const playBtn = document.getElementById("playSongPlaylistBtn");
    ...
    playBtn.addEventListener("click", () => {
        ...
        if (isPlayingPlaylist) {
            playBtn.innerHTML = `<svg width="24" height="24" viewBox="0 0 24 24">
                                    <path d="M9 6l6 6-6 6"/>
                                </svg>`;
            ...
            computedWidth = parseFloat(songLevel.style.width) || 0;
    ...
    const advanceButton = document.getElementById('backToAlbumBtn');
    if (advanceButton) {
        advanceButton.addEventListener('click', function () {
            chooseAlbum(name, xValue, cor);
        });
    }
}
